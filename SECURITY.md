...
**Versioning note:** Current releases use `MAJOR.MINOR.PATCH` (e.g., `3.6.0`). Each row represents the whole patch line (`3.x.x`).
...
* From **3.5.x** → **3.6.0**: Safe upgrade path. Java support added, pb512 output format enhanced (backward compatible). Test cross-language workflows if using Java.
* From **≤ 3.4** → **3.6.0**: upgrade ASAP, **re‑generate keys** and **re‑encrypt** all stored data. Do **not** attempt mixed clusters.
* From **2.9/2.8** → **3.6.0**: plan a one‑way migration with fresh keys and a full re‑encrypt. Validate exports before cutover. Roll back only with full 2.x snapshots (no forward replay).
* From **< 2.6**: treat as potentially breached; rotate credentials, invalidate legacy ciphertext at rest, and perform a clean re‑ingest under **3.6.0**.
