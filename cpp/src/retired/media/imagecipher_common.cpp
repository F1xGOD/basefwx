...
#include "imagecipher_internal.hpp"  // Retired media compatibility only.
...
bool ParseJmgHeader(const Bytes& blob,
    ...
    if (version == basefwx::constants::kJmgKeyVersion) {
        ...
        // The current format prepends one profile byte to the legacy packed
        // key payload. Erasing that already-validated byte avoids expressing
        // the remainder as an iterator range whose synthetic `begin() + 1`
        // path was misdiagnosed as a wrapped allocation by GCC 14.
        key_payload.erase(key_payload.begin());
    }
