...
std::size_t KfmPngBytesPerPixel(int channels) {
    ...
    throw std::invalid_argument("kFM PNG channel count must be 1 or 3");
}
...
void EncodeKfmPixelBlock(std::vector<std::uint8_t>& pixels,
    ...
    const std::size_t bytes_per_pixel = KfmPngBytesPerPixel(channels);
    if (data_len > 0 && data == nullptr) {
        throw std::invalid_argument("kFM PNG block data is null");
    }
    if (pixel_index >
        std::numeric_limits<std::size_t>::max() / bytes_per_pixel) {
        throw std::length_error("kFM PNG pixel offset overflow");
    }
    const std::size_t offset = pixel_index * bytes_per_pixel;
    if (offset > pixels.size()
        || bytes_per_pixel > pixels.size() - offset) {
        throw std::out_of_range("kFM PNG pixel block is out of bounds");
    }
