...
final class B512FileCodec {
    ...
    static File b512FileEncodeFileStream(File input,
        ...
        try {
            ...
            try {
                ...
                boolean fastObf = useFastObfuscation(inputLength);
                    ...
                    try (StreamObfuscator obfuscator =
                        ...
                        try (CryptoBackend.AeadEncryptor enc =
                            ...
                            try {
                                ...
                                out.write(
                                    outBuf, ctLen, Constants.AEAD_TAG_LEN);
                            } finally {
        ...
        try {
            ...
            try (FileInputStream fin = new FileInputStream(input);
                 BufferedInputStream in = new BufferedInputStream(
                         fin, Constants.STREAM_CHUNK_SIZE)) {
                ...
                requireBoundedFileLength(
                        ...
                        Constants.LENGTH_PREFIXED_MAX,
                        "master key transport");
                byte[] masterBlob = readExactBytes(
                        in, lenMaster, "Ciphertext payload truncated");
                int lenPayloadHeader = readU32(
                        in, "Ciphertext payload truncated");
                long lenPayload = resolvePayloadLengthFromFileSize(
                        input, lenUser, lenMaster, lenPayloadHeader);
                long payloadOffset = 12L + lenUser + lenMaster;
                if (lenPayload != input.length() - payloadOffset) {
                    throw new IllegalArgumentException(
                            "Ciphertext payload length does not match "
                            + "remaining file");
                }
                if (lenPayload < 4L + Constants.AEAD_NONCE_LEN
                ...
                metadataBytes = readExactBytes(
                        in, metaLen, "Ciphertext payload truncated");
                if (metadataBytes.length > 0) {
                    metadataBlob = new String(
                            metadataBytes, StandardCharsets.UTF_8);
                }
                if (metadataPreview != null
                        && !metadataPreview.isEmpty()
                        && !metadataPreview.equals(metadataBlob)) {
                    throw new IllegalArgumentException(
                ...
                try {
                    ...
                    try (CryptoBackend.AeadDecryptor dec =
                        ...
                        try (FileOutputStream fout =
                                     new FileOutputStream(tempPlain);
            ...
            try (FileInputStream fin = new FileInputStream(tempPlain);
                 BufferedInputStream plainIn = new BufferedInputStream(
                         fin, Constants.STREAM_CHUNK_SIZE)) {
                if (metadataBytes.length > 0) {
                    byte[] metaBuf = readExactBytes(
                            plainIn,
                            metadataBytes.length,
                            "Metadata integrity mismatch detected");
                    ...
                    byte[] delim = Constants.META_DELIM.getBytes(
                            StandardCharsets.UTF_8);
                    byte[] delimBuf = readExactBytes(
                            plainIn,
                            delim.length,
                            "Malformed streaming payload: missing "
                            + "metadata delimiter");
                ...
                long originalSize = readU64(
                        plainIn,
                        "Malformed streaming payload: missing original size");
                byte[] salt = readExactBytes(
                        plainIn,
                        Constants.STREAM_SALT_LEN,
                ...
                byte[] extBytes = extLen > 0
                        ? readExactBytes(
                                plainIn,
                                extLen,
                                "Malformed streaming payload: truncated "
                                + "extension")
                        : new byte[0];

                File outFile = resolveDecodedOutput(
                        input, output, extBytes);
                stagedOutput = BaseFwxUtil.createPrivateSiblingTempFile(
                ...
                try (StreamObfuscator decoder = obfuscateStream
                     ...
                     BufferedOutputStream out = new BufferedOutputStream(
                             fout, Constants.STREAM_CHUNK_SIZE)) {
                    try {
                ...
                BaseFwxUtil.commitAuthenticatedFile(
                        stagedOutput, outFile);
                stagedOutput = null;
                return outFile;
    ...
    static byte[] b512FileEncodeBytes(byte[] data,
        ...
        if (!enableAead) {
            throw new IllegalArgumentException(
                    "Unauthenticated b512file writing is retired; "
                    + "AES-GCM is required");
        }
    ...
    static BaseFwx.DecodedFile b512FileDecodeBytes(byte[] blob,
                                                  ...
                                                  boolean useMaster,
                                                  boolean stripMetadata) {
        return b512FileDecodeBytes(
                blob,
                password,
                useMaster,
                stripMetadata,
                Constants.envEnabled(
                        "BASEFWX_ALLOW_LEGACY_B512FILE_RAW"));
    }
    ...
    static BaseFwx.DecodedFile b512FileDecodeBytes(byte[] blob,
        ...
        try {
            ...
            try {
                ...
                if (!allowLegacyRaw) {
                    throw new IllegalArgumentException(
                            ...
                            + "BASEFWX_ALLOW_LEGACY_B512FILE_RAW=1 only to "
                            + "recover trusted legacy data",
                            formatFailure);
                }
            ...
            if (binaryParts != null) {
                ...
                try {
                    payloadBytes = Crypto.aesGcmDecrypt(
                            aeadKey,
                            binaryParts.get(2),
                            Constants.B512_AEAD_INFO);
                    content = new String(
