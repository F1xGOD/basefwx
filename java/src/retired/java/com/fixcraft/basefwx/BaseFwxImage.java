...
/** Retired media compatibility surface; excluded from default builds. */
public final class BaseFwxImage {
    ...
    // calls the shared package utilities directly.
    ...
    private static byte[] kfmKeystream(long seed, int length) {
        ...
        BaseFwxUtil.writeU64(state, 0, seed);
        int offset = 0;
        long counter = 0L;
        while (offset < length) {
            BaseFwxUtil.writeU64(state, 8, counter);
    ...
    private static byte[] kfmPackContainer(int mode, byte[] payload, String ext, int flags) {
        ...
        BaseFwxUtil.writeU64(out, 8, payload.length);
        BaseFwxUtil.writeU32(
                out, 16, (int) (crc32.getValue() & 0xFFFFFFFFL));
        BaseFwxUtil.writeU64(out, 20, seed);
        BaseFwxUtil.writeU32(out, 28, 0);
    ...
    private static KfmDecoded kfmUnpackContainer(byte[] blob) {
        ...
        int crcExpected = BaseFwxUtil.readU32(blob, 16);
