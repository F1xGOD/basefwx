...
public final class MediaImageCipher {
    ...
    static File encryptImage(File input,
        ...
        int trailerProfile = RetiredMediaConstants.JMG_SECURITY_PROFILE_LEGACY;
        if (includeTrailer) {
        ...
        if (includeTrailer) {
            if (archiveOriginal) {
                ...
                MediaTrailerCodec.appendBalancedTrailer(output, RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC, trailerBlob);
            } else {
                MediaTrailerCodec.appendBalancedTrailer(output, RetiredMediaConstants.IMAGECIPHER_KEY_TRAILER_MAGIC, trailerHeader);
            }
        }
        return output;
    }
    ...
    static File decryptImage(File input,
        ...
        MediaTrailerCodec.TrailerSplit split = MediaTrailerCodec.splitTrailerForMagic(fileBytes, RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
        byte[] payload = split.payload;
        byte[] trailer = split.trailer;
        byte[] keyTrailer = null;
        if (trailer == null) {
            MediaTrailerCodec.TrailerSplit keySplit = MediaTrailerCodec.splitTrailerForMagic(fileBytes, RetiredMediaConstants.IMAGECIPHER_KEY_TRAILER_MAGIC);
        ...
        if (trailer != null) {
            ...
            if (header != null) {
                ...
                archiveKey = Crypto.hkdfSha256(material, RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO, 32);
                archiveBlob = trailer;
            }
            try {
    ...
    static MaskState buildMaskState(byte[] password,
        ...
        if (material == null) {
            ...
            material = Crypto.pbkdf2HmacSha256(password, RetiredMediaConstants.IMAGECIPHER_STREAM_INFO, iters, 64);
        }
