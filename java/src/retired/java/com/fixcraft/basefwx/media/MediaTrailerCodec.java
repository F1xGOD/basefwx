...
/** Retired jMG compatibility implementation. */
public final class MediaTrailerCodec {
    ...
    static void appendTrailerStream(File output,
        ...
        if (archiveInfo == null || archiveInfo.length == 0) {
            archiveInfo = RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO;
        }
        ...
        try (FileInputStream in = new FileInputStream(original);
            ...
            bufOut.write(RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
    ...
    static boolean decryptTrailerStream(File input,
        ...
        try (RandomAccessFile raf = new RandomAccessFile(input, "r")) {
            byte[] magic = RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC;
            ...
            byte[] prefix = new byte[RetiredMediaConstants.JMG_KEY_MAGIC.length];
            raf.readFully(prefix);

            byte[] archiveKey;
            byte[] archiveInfo = RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO;
            long headerBytes = 0;
            byte[] nonce;
            long cipherBodyLen;
            if (Arrays.equals(prefix, RetiredMediaConstants.JMG_KEY_MAGIC)) {
                headerSeen = true;
                int version = raf.read();
                if (version < 0) {
                    return false;
                }
                if (version != RetiredMediaConstants.JMG_KEY_VERSION_LEGACY && version != RetiredMediaConstants.JMG_KEY_VERSION) {
                ...
                long headerMin =
                        (long) RetiredMediaConstants.JMG_KEY_MAGIC.length + 1L + 4L;
                ...
                int profileId = RetiredMediaConstants.JMG_SECURITY_PROFILE_LEGACY;
                byte[] keyPayload = payload;
                if (version == RetiredMediaConstants.JMG_KEY_VERSION) {
                    if (payload.length < 1) {
                ...
                byte[] maskKey = KeyWrap.recoverMaskKey(parts.get(0), parts.get(1), password, useMaster,
                    RetiredMediaConstants.JMG_MASK_INFO, RetiredMediaConstants.MASK_AAD_JMG, new KeyWrap.KdfOptions("pbkdf2", Constants.USER_KDF_ITERATIONS));
                ...
                archiveKey = Crypto.hkdfSha256(deriveMediaMaterial(password), RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO, 32);
                nonce = new byte[Constants.AEAD_NONCE_LEN];
    ...
    static byte[] decryptTrailer(byte[] fileBytes,
                                         byte[] password,
                                         boolean useMaster) {
        TrailerSplit split = splitTrailerForMagic(fileBytes, RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
    ...
    static TrailerSplit splitTrailer(byte[] data) {
        return splitTrailerForMagic(data, RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
    }
    ...
    static void appendKeyTrailer(File output, byte[] keyHeader) {
        ...
        appendBalancedTrailer(output, RetiredMediaConstants.IMAGECIPHER_KEY_TRAILER_MAGIC, keyHeader);
    }
    ...
    static byte[] loadBaseKeyFromKeyTrailer(File path,
                                                    ...
                                                    boolean useMaster,
                                                    int[] profileOut) {
        TrailerInfo info = extractBalancedTrailerInfo(path, RetiredMediaConstants.IMAGECIPHER_KEY_TRAILER_MAGIC);
        if (info == null) {
            return null;
        }
    ...
    static byte[] loadBaseKeyFromKeyTrailerBytes(byte[] fileBytes,
                                                         ...
                                                         boolean useMaster,
                                                         int[] profileOut) {
        TrailerSplit split = splitTrailerForMagic(fileBytes, RetiredMediaConstants.IMAGECIPHER_KEY_TRAILER_MAGIC);
        if (split.trailer == null) {
            return null;
        }
    ...
    static int normalizeJmgProfile(int profileId) {
        if (profileId == RetiredMediaConstants.JMG_SECURITY_PROFILE_LEGACY
            || profileId == RetiredMediaConstants.JMG_SECURITY_PROFILE_MAX) {
            return profileId;
        }
    ...
    static byte[] jmgStreamInfoForProfile(int profileId) {
        ...
        if (profileId == RetiredMediaConstants.JMG_SECURITY_PROFILE_MAX) {
            return "basefwx.imagecipher.stream.v1.max".getBytes(StandardCharsets.US_ASCII);
        }
        return RetiredMediaConstants.IMAGECIPHER_STREAM_INFO;
    }
    ...
    static byte[] jmgArchiveInfoForProfile(int profileId) {
        ...
        return RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO;
    }
    ...
    static JmgKeys prepareJmgKeys(byte[] password, boolean useMaster) {
        return prepareJmgKeys(password, useMaster, RetiredMediaConstants.JMG_SECURITY_PROFILE_DEFAULT);
    }
    ...
    static JmgKeys prepareJmgKeys(byte[] password, boolean useMaster, int securityProfile) {
        ...
        try (KeyWrap.MaskKeyResult mask = KeyWrap.prepareMaskKey(
                password,
                useMaster,
                RetiredMediaConstants.JMG_MASK_INFO,
                false,
                RetiredMediaConstants.MASK_AAD_JMG,
    ...
    static JmgHeader parseJmgHeader(byte[] blob, byte[] password, boolean useMaster) {
        int headerMin = RetiredMediaConstants.JMG_KEY_MAGIC.length + 1 + 4;
        if (blob.length < headerMin) {
            return null;
        }
        if (!MediaCipherUtil.startsWith(blob, 0, RetiredMediaConstants.JMG_KEY_MAGIC)) {
            return null;
        }
        int version = blob[RetiredMediaConstants.JMG_KEY_MAGIC.length] & 0xFF;
        if (version != RetiredMediaConstants.JMG_KEY_VERSION_LEGACY && version != RetiredMediaConstants.JMG_KEY_VERSION) {
            throw new IllegalArgumentException("Unsupported JMG key header version");
        }
        long payloadLen = MediaCipherUtil.readU32(blob, RetiredMediaConstants.JMG_KEY_MAGIC.length + 1);
    ...
    static byte[] buildJmgHeader(byte[] userBlob, byte[] masterBlob, int securityProfile) {
        ...
        int total = RetiredMediaConstants.JMG_KEY_MAGIC.length + 1 + 4 + payload.length;
        if (total > Constants.FWXAES_MAX_KEY_HEADER_LEN) {
            throw new IllegalArgumentException("JMG key header too large");
        }
        byte[] out = new byte[total];
        int offset = 0;
        System.arraycopy(RetiredMediaConstants.JMG_KEY_MAGIC, 0, out, offset, RetiredMediaConstants.JMG_KEY_MAGIC.length);
        offset += RetiredMediaConstants.JMG_KEY_MAGIC.length;
        out[offset++] = (byte) RetiredMediaConstants.JMG_KEY_VERSION;
    ...
    static byte[] deriveMediaMaterial(byte[] password) {
        ...
        return Crypto.pbkdf2HmacSha256(password, RetiredMediaConstants.IMAGECIPHER_STREAM_INFO, iters, 64);
    }
