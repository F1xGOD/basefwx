...
/** Security regression coverage for the opt-in retired media profile. */
public class MediaTrailerCodecSecurityTest {
    ...
    public void hostileInnerJmgLengthRejectsBeforeAllocationOrOutput()
        ...
        try {
            ...
            byte[] blob = new byte[
                    RetiredMediaConstants.JMG_KEY_MAGIC.length + 1 + 4];
            System.arraycopy(
                    RetiredMediaConstants.JMG_KEY_MAGIC,
                    0,
                    blob,
                    0,
                    RetiredMediaConstants.JMG_KEY_MAGIC.length);
            blob[RetiredMediaConstants.JMG_KEY_MAGIC.length] =
                    (byte) RetiredMediaConstants.JMG_KEY_VERSION;
            writeU32(
                    blob,
                    RetiredMediaConstants.JMG_KEY_MAGIC.length + 1,
                    Integer.MAX_VALUE);
            byte[] length = new byte[4];
            writeU32(length, 0, blob.length);
            try (FileOutputStream out = new FileOutputStream(carrier)) {
                out.write(new byte[] {1, 2, 3, 4});
                out.write(RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
                out.write(length);
                out.write(blob);
                out.write(RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC);
                out.write(length);
    ...
    public void badTagPreservesExistingDestinationAndLeaksNoTempFile()
        ...
        try {
            ...
            MediaTrailerCodec.appendTrailerStream(
                    ...
                    null,
                    new byte[0],
                    RetiredMediaConstants.IMAGECIPHER_ARCHIVE_INFO);
            int footerLength =
                    RetiredMediaConstants.IMAGECIPHER_TRAILER_MAGIC.length + 4;
