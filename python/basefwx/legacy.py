...
class basefwx:
    ...
    @staticmethod
    def an7_file(
        input_path: "basefwx.typing.Union[str, basefwx.pathlib.Path]",
        password: "basefwx.typing.Union[str, bytes, bytearray, memoryview]",
        *,
        out: "basefwx.typing.Optional[basefwx.typing.Union[str, basefwx.pathlib.Path]]" = None,
            ...
            with open(path, "rb") as src, open(temp_path, "wb") as dst:
                ...
                for super_idx in range(superblocks):
                    ...
                    for local in range(block_chunks):
                        ...
                        sha.update(chunk)
                        transformed = basefwx._an7_apply_xor_transform(
                            chunk, keys["stream"], stream_nonce, global_chunk
                        )
                        if (local % 2) == 1:
                ...
                trailer_info = {
                    "format_version": basefwx.AN7_TRAILER_VERSION.decode("ascii"),
                    "original_basename": path.stem,
                    "original_extension": path.suffix,
                    "original_size": payload_len,
                    "chunk_size": basefwx.AN7_CHUNK_SIZE,
                    "superblock_chunks": basefwx.AN7_SUPERBLOCK_CHUNKS,
                    "flip_stride": basefwx.AN7_FLIP_STRIDE,
                    "stream_nonce": stream_nonce,
                    "sha256_original": sha.digest(),
                    "created_utc": basefwx.datetime.now(basefwx.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
                }
                ...
                trailer_crc32 = basefwx.zlib.crc32(encrypted_trailer) & 0xFFFFFFFF
                ...
                footer = (
                    salt
                    + tail_nonce
                    + tail_cipher_and_tag[:basefwx.AN7_TAIL_CIPHER_LEN]
                    + tail_cipher_and_tag[basefwx.AN7_TAIL_CIPHER_LEN:]
                )
                dst.write(footer)
                dst.flush()
    ...
    def dean7_file(
        ...
        *,
        ...
        if (
            trailer_len < (basefwx.AN7_TRAILER_NONCE_LEN + basefwx.AEAD_TAG_LEN)
            or payload_len > file_size
            or (payload_len + trailer_len + basefwx.AN7_FOOTER_SIZE) != file_size
        ):
            raise ValueError("AN7 footer length fields are invalid")

        with open(path, "rb") as src:
            src.seek(payload_len)
            encrypted_trailer = basefwx._an7_read_exact(
                src,
                trailer_len,
                "Failed to read AN7 encrypted trailer",
            )
        if (basefwx.zlib.crc32(encrypted_trailer) & 0xFFFFFFFF) != trailer_crc32:
        ...
        if (
            int(trailer["chunk_size"]) <= 0
            or int(trailer["superblock_chunks"]) <= 0
            or int(trailer["flip_stride"]) <= 0
            or len(trailer["stream_nonce"]) != basefwx.AN7_TRAILER_NONCE_LEN
        ):
        ...
        try:
            with open(path, "rb") as src, open(temp_path, "wb") as dst:
                ...
                for super_idx in range(superblocks):
                    ...
                    chunk_sizes = [
                        basefwx._an7_chunk_bytes_at(payload_len, chunk_size, start_chunk + idx)
                        for idx in range(block_chunks)
                    ]
                    chunks: "list[bytes]" = [b""] * block_chunks
                    order = basefwx._an7_build_permutation(keys["perm"], super_idx, block_chunks)
                    ...
                    for local in range(block_chunks):
                        ...
                        clear_chunk = basefwx._an7_apply_xor_transform(
                            bytes(transformed),
                            keys["stream"],
                            trailer["stream_nonce"],
                            global_chunk,
                        )
        ...
        return {
            ...
            "restored_name": output_path.name,
