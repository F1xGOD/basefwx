...
class BaseFWXUnitTests(unittest.TestCase):
    ...
    def test_cpu_worker_limit_and_telemetry_share_policy(self):
        from basefwx.runtime import _hardware

        with patch.object(_hardware.os, "cpu_count", return_value=12):
            with patch.dict(
                os.environ,
                {"BASEFWX_MAX_THREADS": "2"},
                clear=False,
            ):
                os.environ.pop("BASEFWX_FORCE_SINGLE_THREAD", None)
                workers = _hardware.resolve_worker_count()
            self.assertEqual(workers, 2)
            plan = _hardware.build_cpu_execution_plan(
                "policy-test", workers=workers
            )
            self.assertEqual(plan["parallel_workers"], 2)
            self.assertTrue(plan["parallel_enabled"])

            with patch.dict(
                os.environ,
                {"BASEFWX_FORCE_SINGLE_THREAD": "1"},
                clear=False,
            ):
                self.assertEqual(_hardware.resolve_worker_count(), 1)
    ...
    def test_text_payloads_emit_authenticated_v3_and_reject_tampering(self):
        for name, encode, decode in (
            ('b512', basefwx.b512encode, basefwx.b512decode),
            ('pb512', basefwx.pb512encode, basefwx.pb512decode),
        ):
            with self.subTest(codec=name):
                plaintext = f'{name} authenticated text payload'
                with patch.dict(
                    os.environ,
                    {'BASEFWX_OBFUSCATE_CODECS': '0'},
                    clear=False,
                ):
                    encoded = encode(
                        plaintext, TEST_PASSWORD, use_master=False)
                self.assertTrue(basefwx._looks_like_base64(encoded))
                raw = basefwx.base64.b64decode(encoded, validate=True)
                parts = list(basefwx._unpack_length_prefixed(raw, 3))
                self.assertEqual(parts[2][0], 3)
                self.assertEqual(
                    int.from_bytes(parts[2][1:5], 'big'),
                    len(plaintext.encode('utf-8')),
                )
                self.assertEqual(
                    decode(encoded, TEST_PASSWORD, use_master=False),
                    plaintext,
                )
                self.assertEqual(
                    decode(
                        basefwx.code(encoded),
                        TEST_PASSWORD,
                        use_master=False,
                    ),
                    plaintext,
                ...
                tag_tampered = parts.copy()
                tag_payload = bytearray(tag_tampered[2])
                tag_payload[-1] ^= 1
                tag_tampered[2] = bytes(tag_payload)
                tag_token = basefwx.base64.b64encode(
                    basefwx._pack_length_prefixed(*tag_tampered)
                ).decode('ascii')
                legacy_decoder = (
                    '_b512decode_legacy'
                    if name == 'b512'
                    else '_pb512decode_legacy'
                )
                with (
                    patch.dict(
                        os.environ,
                        {'BASEFWX_ALLOW_LEGACY_CODECS': '1'},
                        clear=False,
                    ),
                    patch.object(
                        basefwx,
                        ...
                        side_effect=AssertionError(
                            'authentication failure downgraded to legacy'
                        ),
                    ),
                ):
                    with self.assertRaisesRegex(
                        ...
                        decode(
                            tag_token, TEST_PASSWORD, use_master=False)

                header_tampered = parts.copy()
                header_payload = bytearray(header_tampered[2])
                header_payload[4] ^= 1
                header_tampered[2] = bytes(header_payload)
                header_token = basefwx.base64.b64encode(
                    basefwx._pack_length_prefixed(*header_tampered)
                ).decode('ascii')
                with self.assertRaisesRegex(ValueError, 'length mismatch'):
                    decode(header_token, TEST_PASSWORD, use_master=False)

    def test_text_payload_domains_are_cryptographically_separated(self):
        from basefwx.file import _b512_memory

        encoded = basefwx.b512encode(
            'domain separation', TEST_PASSWORD, use_master=False)
        raw = basefwx.base64.b64decode(encoded, validate=True)
        user_blob, master_blob, payload = basefwx._unpack_length_prefixed(
            raw, 3)
        ...
        try:
            with self.assertRaisesRegex(ValueError, 'authentication failed'):
                _b512_memory._decode_text_payload(
                    payload,
                    mask_key,
                    stream_info=b'basefwx.b512.stream.v1',
    ...
    def test_text_payload_v2_requires_explicit_recovery_switch(self):
        for name, decode, mask_info, mask_aad, stream_info, required in (
            (
                'b512',
                basefwx.b512decode,
                b'basefwx.b512.mask.v1',
                b'b512',
                b'basefwx.b512.stream.v1',
                False,
            ),
            (
                'pb512',
                basefwx.pb512decode,
                b'basefwx.pb512.mask.v1',
                b'pb512',
                b'basefwx.pb512.stream.v1',
                True,
            ),
        ):
            with self.subTest(codec=name):
                plaintext = f'{name} trusted legacy recovery'
                mask_key_bytes, user_blob, master_blob, _ = (
                ...
                try:
                    ...
                    masked = basefwx._mask_payload(
                        mask_key, clear, info=stream_info)
                    payload = b'\x02' + len(clear).to_bytes(4, 'big') + masked
                    encoded = basefwx.base64.b64encode(
                        basefwx._pack_length_prefixed(
                ...
                with patch.dict(
                    os.environ,
                    {'BASEFWX_ALLOW_LEGACY_TEXT_V2': '0'},
                    clear=False,
                ):
                ...
                with patch.dict(
                    os.environ,
                    {'BASEFWX_ALLOW_LEGACY_TEXT_V2': '1'},
                    clear=False,
                ):
                    self.assertEqual(
                        decode(encoded, TEST_PASSWORD, use_master=False),
    ...
    def test_unauthenticated_b512file_writing_is_retired(self):
        with self.assertRaisesRegex(ValueError, 'writing is retired'):
        ...
        raw_body = (
            basefwx.b512encode(
                '.txt', TEST_PASSWORD, use_master=False)
            + basefwx.FWX_DELIM
            + basefwx.b512encode(
                basefwx.base64.b64encode(b'payload').decode('ascii'),
                TEST_PASSWORD,
                use_master=False,
            )
        ).encode('utf-8')
        with patch.dict(
            ...
            with self.assertRaisesRegex(ValueError, 'disabled'):
                basefwx.b512file_decode_bytes(
                    raw_body, TEST_PASSWORD, use_master=False)
        with patch.dict(
            os.environ,
