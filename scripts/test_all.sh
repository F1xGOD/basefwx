...
_BENCH_AN7_PW = None
_BENCH_AN7_TEMP = None
_BENCH_AN7_SEED_FWX = None
_BENCH_AN7_SEED_AN7 = None
...
def _bench_an7_init(mode: str, input_path: str, pw: str) -> None:
    ...
    _BENCH_AN7_MODE = mode
    _BENCH_AN7_PW = pw
    tmp = tempfile.TemporaryDirectory(prefix="basefwx-bench-an7-")
    _BENCH_AN7_TEMP = tmp
    src = Path(input_path)
    local_input = Path(tmp.name) / src.name
    shutil.copyfile(src, local_input)
    seed_fwx = Path(tmp.name) / "seed.fwx"
...
def _bench_an7_worker(worker_id: int) -> int:
    tmp = Path(_BENCH_AN7_TEMP.name)
    out = tmp / f"bench_{_BENCH_AN7_MODE}_{worker_id}.out"
    try:
        out.unlink()
    except FileNotFoundError:
        pass
...
def cmd_bench_an7(args: list[str]) -> int:
    if len(args) < 2:
        return 2
    inp, pw = args[:2]
    warmup = _warmup_env()
...
def main() -> int:
    ...
    if cmd == "bench-an7":
        return cmd_bench_an7(args)
    if cmd == "bench-dean7":
        return cmd_bench_dean7(args)
...
if [[ -z "$BENCH_FILE_WORKERS" ]]; then
    BENCH_FILE_WORKERS="$BASEFWX_BENCH_WORKERS"
    if [[ -f "$BENCH_BYTES_FILE" ]]; then
        bench_file_size="$(wc -c < "$BENCH_BYTES_FILE" | tr -d ' ')"
        if [[ "$bench_file_size" =~ ^[0-9]+$ && "$bench_file_size" -gt 0 ]]; then
            BENCH_FILE_TMP_BUDGET_BYTES="${BENCH_FILE_TMP_BUDGET_BYTES:-2147483648}"
            if [[ ! "$BENCH_FILE_TMP_BUDGET_BYTES" =~ ^[0-9]+$ || "$BENCH_FILE_TMP_BUDGET_BYTES" -lt 1 ]]; then
                BENCH_FILE_TMP_BUDGET_BYTES=2147483648
            fi
            per_worker=$((bench_file_size * 3))
            if (( per_worker > 0 )); then
                max_workers=$((BENCH_FILE_TMP_BUDGET_BYTES / per_worker))
                if (( max_workers < 1 )); then
                    max_workers=1
                fi
                if (( BENCH_FILE_WORKERS > max_workers )); then
                    BENCH_FILE_WORKERS=$max_workers
                fi
            fi
        fi
    fi
...
if [[ -n "${BASEFWX_MAX_THREADS:-}" ]]; then
    log "BENCH_MAX_THREADS: $BASEFWX_MAX_THREADS"
fi
...
with open(tsv_path, "r", encoding="utf-8") as handle:
    for line in handle:
        ...
        if len(parts) < 5:
            parts += [""] * (5 - len(parts))
        label, py, pypy, cpp, java = parts[:5]
        times = {}
