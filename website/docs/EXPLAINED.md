---
layout: "doc"
title: "BaseFWX explained"
description: "File encryption, unlock paths, portable variants, live streams, and failure handling."
permalink: "/docs/EXPLAINED/"
generated_from: "docs/EXPLAINED.md"
doc_source: "docs/src/en_US/pages/explained.doc"
lang: "en-US"
doc_locale: "en_US"
---
...
BaseFWX provides authenticated containers in C++, Python, and Java. The
runtimes share the portable format variants, while the compatibility reference
records runtime-specific variants and packing limits. This page gives the
working model without duplicating the CLI or byte-level reference.

## File encryption

The `fwxAES` path has four steps:

<!-- yume-diagram: file_encryption -->
{% raw %}
```text
+----------------------------------+
|  PLAINTEXT                       |
...
AES-GCM encrypts the fwxAES payload under a fixed `fwxAES` additional
authenticated data (AAD) value. The format and key-wrap headers are not part of
that AAD, so the parser validates their structure and bounds separately. A
reader verifies the payload tag before it publishes plaintext. A wrong
password, changed ciphertext, truncated body, or changed tag fails decryption.
...
When an unrelated configured master key produces a candidate that fails payload
...
Portable format variants are a contract across C++, Python, and Java. The C++
unsized or greater-than-4-GiB fwxAES stream variant is C++-only, and Java
refuses packed `ENC-P` containers. A format or security change must update the
affected runtimes and shared known-answer tests in the same change unless the
API is explicitly documented as runtime-specific.
...
release. See [ABI.md]({{ '/docs/ABI/' | relative_url }}) and [COMPATIBILITY.md]({{ '/docs/COMPATIBILITY/' | relative_url }}).
